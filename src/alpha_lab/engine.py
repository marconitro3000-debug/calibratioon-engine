...
from .metrics import decay_curve, summarize
...
class ResearchEngine:
    ...
    def calibrate_alpha(
        self, alpha_template: str, data: dict, param_space: dict, *, optimizer="auto", max_evals=500, objective=None
    ) -> AlphaResult:
        cal = AlphaCalibrator(
            costs=self.costs,
            neutralization=self.neutralization,
...
class AutoAlphaResearchEngine(ResearchEngine):
    def research(
        ...
        *,
        ...
        if param_space:
            return self.calibrate_alpha(
                alpha_template, data, param_space, optimizer=optimizer, max_evals=max_evals, objective=objective
            )
